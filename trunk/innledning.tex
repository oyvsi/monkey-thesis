...
På grunn av en naturlig sammenheng og avhengigheter mellom kravene som er satt, har vi valgt å gruppere funksjonalitet i moduler. De forskjellige modulene er ``Kjerneprogramvare'', ``Server'', ``Infrastruktur'', ''Servermiljø'', ``Varsling'' og ``Statusvindu'' (visualisert i Vedlegg \ref[p.~12]{app:forprosjekt}). Med nærliggende sammenheng mener vi at funksjonalitet som CPU, RAM, disk og prosesser er i samme kategori. 
...
Kapittel 2 gir en teoretisk bakgrunn.
Kapittel 3 handler om implementasjonen som ble utført.
Kapittel 4 går gjennom sikkerhetsvurderinger i implementasjonen.
Kapittel 5 dokumentasjon gruppen anser vil være nyttig.
Kapittel 6 avsluttes og konkluderes rapporten.
\subsection{Konvensjoner} %TODO
